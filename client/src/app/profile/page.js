...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchUser = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER}/user-profile`,
          {
    ...
    const fetchInfluencers = async () => {
      try {
        const res = await axios.get(
          `${process.env.NEXT_PUBLIC_SERVER}/get-influencers`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        );
  ...
  const handleSelectInfluencer = async (influencer) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER}/check-balance`,
        {
          type: callMode,
          duration: 1, // Initial check for 1 minute
          influencerId: influencer.id,
        },
        {
          headers: {
      ...
      if (response.data.message === "Sufficient balance") {
  ...
  const endCall = async () => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_SERVER}/make-call`,
        {
          type: activeCall.callType,
          duration,
          influencerId: activeCall.influencer._id,
        },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      ...
      // Update user's wallet balance in the UI
      const rate = activeCall.influencer.ratePerMinute[activeCall.callType];
      ...
      alert("Error processing call. Please check your balance.");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50">
      ...
      {activeCall && (
        <VideoCall
          influencer={activeCall.influencer}
        ...
        {editMode ? (
          <div className="flex flex-col gap-5">
            ...
            {user.role === "influencer" && (
              <>
                <h2 className="text-2xl font-semibold text-gray-800 mt-6 mb-4 border-b pb-2">
                  Influencer Details
                </h2>
                <label className="block">
                  <span className="text-gray-700 font-medium mb-1 block">
